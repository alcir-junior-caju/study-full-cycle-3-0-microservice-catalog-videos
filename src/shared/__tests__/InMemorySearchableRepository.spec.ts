...
class StubEntity extends Entity {
  ...
  constructor({ entityId, name, price }: StubEntityConstructorParams) {
    super();
...
describe('InMemorySearchableRepository Tests', () => {
  ...
  describe('setFilter method', () => {
    ...
    it('should filter using a filter param', async () => {
      ...
      const spyFilterMethod = jest.spyOn(items, 'filter' as any);
      let itemsFiltered = await repository['setFilter'](items, 'TEST');

      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(1);

      itemsFiltered = await repository['setFilter'](items, '5');
      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(2);

      itemsFiltered = await repository['setFilter'](items, 'no-filter');
  ...
  describe('setSort method', () => {
    it('should no sort items', async () => {
      ...
      let itemsSorted = repository['setSort'](items, null, null);
      expect(itemsSorted).toStrictEqual(items);

      itemsSorted = repository['setSort'](items, 'price', 'asc');
      expect(itemsSorted).toStrictEqual(items);
    });
  ...
  describe('setPaginate method', () => {
    it('should paginate items', async () => {
      ...
      let itemsPaginated = repository['setPaginate'](items, 1, 2);
      expect(itemsPaginated).toStrictEqual([items[0], items[1]]);

      itemsPaginated = repository['setPaginate'](items, 2, 2);
      expect(itemsPaginated).toStrictEqual([items[2], items[3]]);

      itemsPaginated = repository['setPaginate'](items, 3, 2);
      expect(itemsPaginated).toStrictEqual([items[4]]);

      itemsPaginated = repository['setPaginate'](items, 4, 2);
